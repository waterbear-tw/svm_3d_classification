...
# 計算分隔超平面的 z 值
# 超平面方程式： coef[0] * x1 + coef[1] * x2 + coef[2] * x3 + intercept = 0
# 整理得到 x3 = -(coef[0] * x1 + coef[1] * x2 + intercept) / coef[2]

x1_grid, x2_grid = np.meshgrid(np.linspace(-20, 20, 50), np.linspace(-20, 20, 50))
x3_grid = -(coef[0, 0] * x1_grid + coef[0, 1] * x2_grid + intercept[0]) / coef[0, 2]
...
# 設定 3D 圖的標題和軸標籤
fig.update_layout(
    title="SVM Decision Boundary in 3D with Adjustable Distance Threshold",
    scene=dict(
        xaxis_title="X1",
        yaxis_title="X2",
